...
def fix_path(file_path: str, code_rect: tuple[float, float, float, float], ref_month_rect: tuple[float, float, float, float], page_num = 0, regex: str | None = None):
  if page_num < 0:
    return file_path
  words = get_document_words(file_path, page=page_num)
  raw_code = get_words(words, code_rect, regex)[-1].text.split('-')[0]
...
def main(search_paths: list[str], code_rect: tuple[float, float, float, float], ref_month_rect: tuple[float, float, float, float], page_filter: str | None = None, regex: str | None = None):
